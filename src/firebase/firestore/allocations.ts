'use client';
import { collection, query, where, getDocs, Query, onSnapshot } from 'firebase/firestore';
...
export function useAllocations() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(allocationsQuery, 
        (snapshot) => {
            const allocs = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as Allocation));
...
export function useAcceptedApplications() {
    ...
    const allocatedApplicationIds = useMemo(() => new Set(allocations?.map(a => a.applicationId) || []), [allocations]);
    
    const acceptedApps = useMemo(() => {
      return applications?.filter(app => app.status === 'Принята' && !allocatedApplicationIds.has(app.id)) || [];
